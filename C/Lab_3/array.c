...
//Answers to Questions in tpsAnswers
...
int* bubbleSort(int arr[], int n) {
    ...
    for (i = 0; i < n - 1; i++){
        for (j = 0; j < n-1; j++){
            //Basically replaces everything with pointer notation
            //Does the Swap here
            if(*(s_arr+j) > *(s_arr+j+1)){
                temp = *(s_arr+j+1);
                *(s_arr+j+1) = *(s_arr+j);
...
int bSearch(int *arr, int a, int b, int key) {
    ...
    // Your code goes here:
    // Half of the size 
    //Got Help from Austin
    int half = (a+b)/2 ;
    //If the key is equal to dereferenced arr value plus half
    if(key == *(arr+half)){
        return half;   
    }
    //If the key is greater than deferenced arr+half, goes to top half
    if(key > *(arr+half)){
        return bSearch(arr, half+1, b, key);
    }
    //If the key is less than deferenced arr+half, goes to bottom half
    if(key < *(arr+half)){
