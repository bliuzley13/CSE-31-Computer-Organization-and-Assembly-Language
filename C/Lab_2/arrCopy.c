...
void arrCopy(){
	// Your code here
    //Meant to add a position in the even/odd arrays
    int even = 0;
    int odd = 0;
    //First for loop for even
    for (int i = 0; i < size; i++){
        //*(arr+i) works since it points to arr and i does the shift in the position
        //and it gets the value which is modulus 2
        if (*(arr+i) % 2 == 0){
            //Starts off at 0 because of 0th index for arr_even
            *(arr_even+even) = *(arr+i);
            //Adds 1 to the even array in position
            even++;
        }
    }
    //First for loop for odd
    for (int i = 0; i < size; i++){
        if (*(arr+i) % 2 == 1){
            *(arr_odd+odd) = *(arr+i);
            odd++;
    ...
    //Cannot put if statements in one because it messes up the *(arr+i)
    //which would iterate twice
}
...
int main(){
	...
	// Your code here
    for (int i = 0; i < size; i++){
        //Prints #1, #2, #3 when looping
        printf("Enter array element #%d: ", i+1);
        scanf("%d", arr+i);
        int *value = arr+i;
        //Meant to sort the even or odd and allocates memory for it
        if (*value % 2 == 0){
            evenCount++;
        }
        else{
